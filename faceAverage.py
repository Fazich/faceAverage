...
if __name__ == '__main__':
    ...
    for i in xrange(0, numImages):
        ...
        eyecornerSrc = [allPoints[i][39], allPoints[i][38]];
        ...
        points2 = np.reshape(np.array(points1), (83, 1, 2));

        points = cv2.transform(points2, tform);

        points = np.float32(np.reshape(points, (83, 2)));
    ...
    for i in xrange(0, len(imagesNorm)):
        ...
        output = output + img;
        eachOutPut = output/(i+1)

        print '第'+str(i)+'次绘制开始'
        cv2.imshow('image', eachOutPut);
        cv2.waitKey(1);
